...
class CanvasFallbackRenderer {
  ...
  render(snapshot, dt = 0.016) {
    ...
    const danger = snapshot.dangerLevel || 0;
    const storm = snapshot.stormStrength || 0;
    const cameraShake = snapshot.cameraShake || 0;
    const horizonY = height * (0.2 + phase * 0.05);
    const worldToScreenX = (x) => width * 0.5 + x * width * 0.055;
    const worldToScreenY = (y) => {
      const relativeY = y - snapshot.player.y;
      const ahead = 52;
      const behind = 12;
      return height * (1 - (relativeY + behind) / (ahead + behind));
    };

    ctx.save();
    ctx.clearRect(0, 0, width, height);
    ctx.translate(
      Math.sin(this.elapsed * 21) * cameraShake * 16,
      Math.cos(this.elapsed * 18) * cameraShake * 22
    );

    const skyGradient = ctx.createLinearGradient(0, 0, 0, height);
    skyGradient.addColorStop(0, `rgb(${Math.round(130 + (1 - phase) * 42)}, ${Math.round(185 - phase * 54)}, ${Math.round(228 - phase * 64)})`);
    skyGradient.addColorStop(0.5, `rgb(${Math.round(48 + phase * 34)}, ${Math.round(82 - phase * 12)}, ${Math.round(116 - phase * 20)})`);
    skyGradient.addColorStop(1, `rgb(${Math.round(10 + phase * 28)}, ${Math.round(18 + phase * 6)}, ${Math.round(24 - phase * 2)})`);
    ctx.fillStyle = skyGradient;
    ctx.fillRect(0, 0, width, height);

    const moonGlow = ctx.createRadialGradient(width * 0.23, horizonY * 0.6, 0, width * 0.23, horizonY * 0.6, width * 0.16);
    moonGlow.addColorStop(0, `rgba(234, 248, 255, ${0.24 + (1 - phase) * 0.2})`);
    moonGlow.addColorStop(1, 'rgba(234, 248, 255, 0)');
    ctx.fillStyle = moonGlow;
    ctx.fillRect(0, 0, width, height);

    ctx.globalAlpha = 0.8 - phase * 0.5;
    for (let index = 0; index < 90; index += 1) {
      const x = ((index * 137.5) % width);
      const y = ((index * 71.3) % Math.max(1, horizonY));
    ...
    floorGradient.addColorStop(0, `rgba(${Math.round(30 + phase * 44)}, ${Math.round(46 - phase * 8)}, ${Math.round(58 - phase * 16)}, 0.92)`);
    floorGradient.addColorStop(1, `rgba(${Math.round(4 + phase * 24)}, ${Math.round(8 + phase * 8)}, ${Math.round(12 + phase * 6)}, 1)`);
    ctx.fillStyle = floorGradient;
    ctx.fillRect(0, horizonY, width, height - horizonY);

    const centerX = width * 0.5 + snapshot.player.x * width * 0.018;
    const baseWidth = width * 0.54;
    const topWidth = width * 0.13;

    const leftWall = ctx.createLinearGradient(0, horizonY, width * 0.3, height);
    leftWall.addColorStop(0, `rgba(${Math.round(78 + (1 - phase) * 46)}, ${Math.round(102 + (1 - phase) * 38)}, ${Math.round(120 + (1 - phase) * 28)}, 0.88)`);
    leftWall.addColorStop(1, `rgba(${Math.round(18 + phase * 52)}, ${Math.round(24 + phase * 18)}, ${Math.round(32 + phase * 10)}, 1)`);
    ctx.fillStyle = leftWall;
    ctx.beginPath();
    ctx.moveTo(centerX - topWidth, horizonY);
    ctx.lineTo(centerX - baseWidth, height);
    ctx.lineTo(0, height);
    ctx.lineTo(0, horizonY * 0.9);
    ctx.closePath();
    ctx.fill();

    const rightWall = ctx.createLinearGradient(width, horizonY, width * 0.7, height);
    rightWall.addColorStop(0, `rgba(${Math.round(62 + (1 - phase) * 34)}, ${Math.round(84 + (1 - phase) * 28)}, ${Math.round(102 + (1 - phase) * 24)}, 0.86)`);
    rightWall.addColorStop(1, `rgba(${Math.round(16 + phase * 48)}, ${Math.round(20 + phase * 14)}, ${Math.round(28 + phase * 12)}, 1)`);
    ctx.fillStyle = rightWall;
    ctx.beginPath();
    ctx.moveTo(centerX + topWidth, horizonY);
    ctx.lineTo(centerX + baseWidth, height);
    ctx.lineTo(width, height);
    ctx.lineTo(width, horizonY * 0.92);
    ctx.closePath();
    ctx.fill();

    ctx.strokeStyle = `rgba(248, 252, 255, ${0.72 - phase * 0.16})`;
    ctx.lineWidth = Math.max(2, width * 0.0022);
    ctx.beginPath();
    ctx.moveTo(centerX, height);
    ctx.quadraticCurveTo(
      centerX - snapshot.player.x * width * 0.015,
      height * 0.56,
      width * 0.5,
    ...
    for (let lane = -1; lane <= 1; lane += 1) {
      const laneX = worldToScreenX(lane * 2.85);
      ctx.strokeStyle = `rgba(220, 235, 246, ${lane === 0 ? 0.18 : 0.1})`;
      ctx.lineWidth = 1;
      ctx.setLineDash([8, 16]);
      ctx.beginPath();
      ctx.moveTo(laneX, height * 0.4);
    ...
    snapshot.footprints.forEach((mark) => {
      const x = worldToScreenX(mark.x);
      const y = worldToScreenY(mark.y);
      const alpha = Math.max(0, 0.28 - mark.age * 0.14);
      ctx.save();
      ctx.translate(x, y);
      ctx.rotate(mark.rotation);
      ctx.fillStyle = `rgba(220, 236, 246, ${alpha})`;
      ctx.beginPath();
      ctx.ellipse(0, 0, width * 0.0045, height * 0.012, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
    });

    snapshot.avalanches.forEach((avalanche, index) => {
      const y = worldToScreenY(avalanche.y);
      const alpha = 0.14 + avalanche.intensity * 0.22;
      const bandHeight = height * 0.03 * avalanche.heightScale;
      const drift = Math.sin(this.elapsed * 2.4 + index) * width * 0.03;

      const cloudGradient = ctx.createLinearGradient(0, y - bandHeight, 0, y + bandHeight * 2);
      cloudGradient.addColorStop(0, `rgba(255, 255, 255, ${alpha * 0.15})`);
      cloudGradient.addColorStop(0.45, `rgba(228, 245, 255, ${alpha})`);
      cloudGradient.addColorStop(1, 'rgba(255, 255, 255, 0)');
      ctx.fillStyle = cloudGradient;
      ctx.beginPath();
      ctx.moveTo(-40, y + bandHeight);
      ctx.quadraticCurveTo(width * 0.25 + drift, y - bandHeight * 1.6, width * 0.5, y);
      ctx.quadraticCurveTo(width * 0.75 - drift, y + bandHeight * 1.8, width + 40, y - bandHeight * 0.2);
      ctx.lineTo(width + 40, y + bandHeight * 2.4);
      ctx.lineTo(-40, y + bandHeight * 2.8);
      ctx.closePath();
      ctx.fill();
    });
    ...
    snapshot.rocks.forEach((rock, index) => {
      ...
      const y = worldToScreenY(rock.y);
      const radius = Math.max(width * 0.012, width * 0.015 * rock.size);
      const trail = ctx.createLinearGradient(x, y - radius * 4, x, y + radius);
      trail.addColorStop(0, `rgba(255, 214, 170, ${rock.warning ? 0.22 : 0.1})`);
      trail.addColorStop(1, 'rgba(255, 214, 170, 0)');
      ctx.fillStyle = trail;
      ctx.beginPath();
      ctx.ellipse(x, y - radius * 1.8, radius * 0.65, radius * 2.3, 0, 0, Math.PI * 2);
      ctx.fill();

      ctx.save();
      ctx.translate(x, y);
      ctx.rotate(this.elapsed * 3.2 + index);
      ctx.fillStyle = rock.warning ? '#9fc7df' : '#4d6471';
      ctx.beginPath();
      for (let point = 0; point < 7; point += 1) {
        const angle = (Math.PI * 2 * point) / 7;
        const pointRadius = radius * (0.78 + ((point + index) % 3) * 0.18);
    ...
    const playerX = worldToScreenX(snapshot.player.x);
    const playerY = worldToScreenY(snapshot.player.y);

    ctx.strokeStyle = 'rgba(255, 247, 233, 0.8)';
    ctx.lineWidth = Math.max(2, width * 0.0018);
    ctx.beginPath();
    ctx.moveTo(width * 0.5, horizonY + height * 0.01);
    ctx.quadraticCurveTo(width * 0.49 + snapshot.player.x * width * 0.012, playerY - height * 0.12, playerX, playerY - height * 0.04);
    ctx.stroke();

    if (snapshot.player.shieldActive) {
      const shieldGradient = ctx.createRadialGradient(playerX, playerY - height * 0.05, width * 0.01, playerX, playerY - height * 0.05, width * 0.08);
      shieldGradient.addColorStop(0, 'rgba(184, 243, 255, 0.28)');
      shieldGradient.addColorStop(1, 'rgba(184, 243, 255, 0)');
      ctx.fillStyle = shieldGradient;
      ctx.beginPath();
      ctx.ellipse(playerX, playerY - height * 0.05, width * 0.05, height * 0.09, 0, 0, Math.PI * 2);
      ctx.fill();
    }

    ctx.save();
    ctx.translate(playerX, playerY);
    const lean = Math.max(-0.45, Math.min(0.45, snapshot.player.x * 0.1 + (snapshot.player.vx || 0) * 0.006));
    ctx.rotate(snapshot.player.falling ? -1.05 : -lean * 0.8);
    if (snapshot.player.falling) {
      ctx.translate(0, snapshot.player.fallOffset * 0.9);
    }

    ctx.strokeStyle = '#f4f9fd';
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ctx.lineWidth = Math.max(3, width * 0.0036);
    ctx.beginPath();
    ctx.moveTo(0, -height * 0.03);
    ctx.lineTo(0, -height * 0.085);
    ctx.stroke();

    ctx.lineWidth = Math.max(4, width * 0.0042);
    ctx.beginPath();
    ctx.moveTo(0, -height * 0.07);
    ctx.lineTo(-width * 0.018, -height * 0.035);
    ctx.moveTo(0, -height * 0.07);
    ctx.lineTo(width * 0.018, -height * 0.045);
    ctx.moveTo(0, -height * 0.03);
    ctx.lineTo(-width * 0.014, height * 0.02);
    ctx.moveTo(0, -height * 0.03);
    ctx.lineTo(width * 0.016, height * 0.024);
    ctx.stroke();

    ctx.fillStyle = '#ffb56c';
    ctx.beginPath();
    ctx.arc(0, -height * 0.095, Math.max(6, width * 0.008), 0, Math.PI * 2);
    ctx.fill();

    ctx.restore();

    ctx.fillStyle = `rgba(255, 255, 255, ${0.08 + storm * 0.18})`;
    for (let index = 0; index < 240; index += 1) {
      const x = (index * 53 + this.elapsed * 180 * (1 + storm)) % (width + 120) - 60;
      const y = (index * 29 + this.elapsed * 420 * (1 + storm * 0.6)) % (height + 120) - 60;
      const length = 8 + storm * 12;
      ctx.beginPath();
      ctx.moveTo(x, y);
      ctx.lineTo(x - length * 0.45, y + length);
      ctx.strokeStyle = `rgba(255,255,255,${0.06 + storm * 0.16})`;
      ctx.lineWidth = 1;
      ctx.stroke();
    }

    if (phase > 0.6) {
      ctx.fillStyle = `rgba(255, 130, 76, ${0.07 + phase * 0.12})`;
      for (let index = 0; index < 90; index += 1) {
        const x = ((index * 97) % width);
        const y = height - ((index * 41 + this.elapsed * 120) % Math.max(1, height * 0.58));
    ...
    const vignette = ctx.createRadialGradient(width * 0.5, height * 0.4, width * 0.12, width * 0.5, height * 0.45, width * 0.72);
    vignette.addColorStop(0, 'rgba(0, 0, 0, 0)');
    vignette.addColorStop(1, `rgba(0, 0, 0, ${0.28 + danger * 0.18})`);
...
function createGameRenderer(canvas) {
  if (typeof THREE === 'undefined') {
    console.warn('THREE is unavailable, using fallback canvas renderer.');
    return new CanvasFallbackRenderer(canvas);
  }

  try {
    return new BergRenderer(canvas);
  } catch (error) {
    console.warn('WebGL renderer initialization failed, using fallback canvas renderer.', error);
    return new CanvasFallbackRenderer(canvas);
  }
}
