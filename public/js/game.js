...
// Hold rock waves at least 15 seconds apart on every device so the
// reaction window never collapses, regardless of platform or frame rate.
const ROCK_MIN_CADENCE_MS = 15000;
...
class Game {
  ...
  _spawnRockWave(now) {
    ...
    this.nextRockSpawnAt = now + Math.max(ROCK_MIN_CADENCE_MS, cadence);
